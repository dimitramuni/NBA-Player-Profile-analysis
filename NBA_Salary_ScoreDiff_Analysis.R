...
##reading three seasons gamelog data including a new column called HomeAway describing
...
team_tags_9900<-c('MIA','NYK','PHI','ORL','BOS','NJN','WAS','IND','CHH','TOR','DET','MIL','CLE','ATL',
...
team_tags_0304<-c('ATL','BOS','CHI','CLE','DAL','DEN','DET','GSW','HOU','IND','LAC',
                  'LAL','MEM','MIA','MIL','MIN','NJN','NOH','NYK','ORL','PHI','PHO',
...
team_tags_0506<-c('ATL','BOS','CHA','CHI','CLE','DAL','DEN','DET','GSW','HOU','IND','LAC',
                  'LAL','MEM','MIA','MIL','MIN','NJN','NOK','NYK','ORL','PHI','PHO',
                  'POR','SAC','SAS','SEA','TOR','UTA','WAS')
...
#creating a function that takes gamelog and total salary (for each team) and season (string for graphing purpose)
#it return two ggplot object returned as a list that plots linear regression line for Score Difference 
...
plot_regression<-function(gamelog,salary,season){
  ...
  #creating a new variable ScorePM which indicates difference in score between Portland and Opponent team
  season_gamelog$ScorePM=season_gamelog$TmScore-season_gamelog$OppScore
  
  #creating new variable which is log of ratio of total salary of Portland and Opponent team
  season_gamelog$log_diff_salary=NA
  for (ind in 1:dim(season_gamelog)[1]) {
    
    #log(Portland_total_salary/ OpponentTeam_total_salary)= log(Portland_total_salary)- log(OpponentTeam_total_salary)
  ...
  #identifying matches which are home or away and storing their indices for later use
  home_inds=which(season_gamelog$HomeAway=='Home')
  away_inds=which(season_gamelog$HomeAway=='Away')
  
  #using linear regression function lm, using log of salary difference as covariate and ScorePM predicator
  ...
  #using ggplot to visualise scatter plot of logarithmic salary difference and score difference
  #plotting linear regression line to visualise overall trend of score difference with change in ratio.
  
  #first plot focuses on home games, using subset function to select only home games for scatter plot and regression line
  ...
  #second plot focuses on away games, using subset function to select only home games for scatter plot and regression line
  p2<-ggplot(subset(season_gamelog, HomeAway %in% 'Away' ),
    ...
    ylab('Score Difference')+
    #using season variable for plotting season name (in a rather naive fashion!!)
